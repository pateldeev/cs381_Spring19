...
//---------------------------------------------------------------------------
BaseApplication::BaseApplication(const std::string &window_name) :
	mRoot(nullptr), mCamera(nullptr), mSceneMgr(nullptr), mWindow(nullptr), mResourcesCfg(Ogre::StringUtil::BLANK),
	mPluginsCfg(Ogre::StringUtil::BLANK), mOverlaySystem(nullptr), mTrayMgr(nullptr), mDetailsPanel(nullptr),
	mCursorWasVisible(false), mShutDown(false), mInputManager(nullptr), mMouse(nullptr), mKeyboard(nullptr), mWindowName(window_name) {
#if OGRE_PLATFORM == OGRE_PLATFORM_APPLE
...
void BaseApplication::createFrameListener(void) {
	...
	pl.insert(std::make_pair(std::string("x11_mouse_hide"), std::string("false"))); //show mouse - only for linux
