...
SoundMgr* SoundMgr::mSoundManager = NULL;
...
/****************************************************************************/
void SoundMgr::selfDestruct(void) {
	if (getSingletonPtr())
		delete getSingletonPtr();
}

SoundMgr* SoundMgr::getSingletonPtr(void) {
...
void SoundMgr::setAudioPath(char* path) {
	mAudioPath = std::string(path);
}
...
/*****************************************************************************/
bool SoundMgr::loadAudio(std::string filename, unsigned int *audioId, bool loop) {
	...
	while (mAudioSourceInUse[sourceID])
		sourceID++;
...
int SoundMgr::loadAudioInToSystem(std::string filename) {
	...
	while (mAudioBufferInUse[bufferID] == true)
		bufferID++;
...
bool SoundMgr::resumeAllAudio(void) {
	...
	for (unsigned int i = 0; i < mAudioSourcesInUseCount; i++) {
		// Are we currently playing the audio source?
...
/****************************************************************************/
bool SoundMgr::setSound(unsigned int audioID, Ogre::Vector3 position, Ogre::Vector3 velocity, Ogre::Vector3 direction, float maxDistance,
...
/****************************************************************************/
bool SoundMgr::setSoundPosition(unsigned int audioID, Ogre::Vector3 position, Ogre::Vector3 velocity, Ogre::Vector3 direction) {
	if (audioID >= MAX_AUDIO_SOURCES || !mAudioSourceInUse[audioID])
		return false;
...
/****************************************************************************/
bool SoundMgr::setListenerPosition(Ogre::Vector3 position, Ogre::Vector3 velocity, Ogre::Quaternion orientation) {
	Ogre::Vector3 axis;
