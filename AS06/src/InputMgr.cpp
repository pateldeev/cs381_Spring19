...
InputMgr::InputMgr(Engine *engine) :
	...
	m_select_start_x = m_select_start_y = m_select_end_x = m_select_end_y = 0.f;
}
...
void InputMgr::UpdateCamera(float dt) {
	const float move_slow = 300.0f, move_fast = 600.0f;
	const float rotate_slow = 0.15f, rotate_fast = 0.25f;

	Ogre::Vector3 move_direction = Ogre::Vector3::ZERO;
	Entity381 *camera_attached_to = cameraFollowing;
	cameraFollowing = 0;

	if (mKeyboard->isKeyDown(OIS::KC_A))
		move_direction.x -= (mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? move_fast : move_slow;
	else if (mKeyboard->isKeyDown(OIS::KC_D))
		move_direction.x += (mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? move_fast : move_slow;
	else if (mKeyboard->isKeyDown(OIS::KC_W))
		move_direction.z -= (mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? move_fast : move_slow;
	else if (mKeyboard->isKeyDown(OIS::KC_S))
		move_direction.z += (mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? move_fast : move_slow;
	else if (mKeyboard->isKeyDown(OIS::KC_R))
		move_direction.y += (mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? move_fast : move_slow;
	else if (mKeyboard->isKeyDown(OIS::KC_F))
		move_direction.y -= (mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? move_fast : move_slow;
	else if (mKeyboard->isKeyDown(OIS::KC_Q))
		cameraNode->yaw(Ogre::Degree((mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? rotate_fast : rotate_slow));
	else if (mKeyboard->isKeyDown(OIS::KC_E))
		cameraNode->yaw(-1 * Ogre::Degree((mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? rotate_fast : rotate_slow));
	else if (mKeyboard->isKeyDown(OIS::KC_Z))
		cameraNode->pitch(Ogre::Degree((mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? rotate_fast : rotate_slow));
	else if (mKeyboard->isKeyDown(OIS::KC_X))
		cameraNode->pitch(-1 * Ogre::Degree((mKeyboard->isKeyDown(OIS::KC_LSHIFT)) ? rotate_fast : rotate_slow));
	else
		cameraFollowing = camera_attached_to;

	if (mKeyboard->isKeyDown(OIS::KC_R) && mKeyboard->isKeyDown(OIS::KC_LCONTROL)) { //reset camera position
	...
	if (cameraFollowing) { //follow entity
		...
		cameraNode->translate(move_direction * dt, Ogre::Node::TS_LOCAL);
...
bool InputMgr::mouseMoved(const OIS::MouseEvent& me) {
	if (mMouse->getMouseState().buttonDown(OIS::MouseButtonID::MB_Left)) {
		m_select_end_x = float(me.state.X.abs);
		m_select_end_y = float(me.state.Y.abs);
	}
	return true;
}
...
bool InputMgr::mousePressed(const OIS::MouseEvent &me, OIS::MouseButtonID mid) {
	if (mid == OIS::MouseButtonID::MB_Left || mid == OIS::MouseButtonID::MB_Right || mid == OIS::MouseButtonID::MB_Middle) {
		...
		if (groundIntersection.first) { //we have an intersection with ocean
			...
			if (nearestEntIndex >= 0 && mid == OIS::MouseButtonID::MB_Left && !mKeyboard->isKeyDown(OIS::KC_RSHIFT)) { //select clicked object
			...
			} else if (nearestEntIndex >= 0 && mid == OIS::MouseButtonID::MB_Right) { //intercept/follow clicked object
				...
				if (engine->entityMgr->IsEntitySelected(engine->entityMgr->GetEntity(nearestEntIndex))) {
					...
					if (mKeyboard->isKeyDown(OIS::KC_LCONTROL)) { //follow
						...
						engine->entityMgr->AddFollowCommandToSelectedEntities(engine->entityMgr->GetEntity(nearestEntIndex),
							!mKeyboard->isKeyDown(OIS::KC_LSHIFT));
					} else { //intercept
						std::cout << std::endl << "SUCCESS: Now having selected entities intercept clicked entity!" << std::endl;
						engine->entityMgr->AddInterceptCommandToSelectedEntities(engine->entityMgr->GetEntity(nearestEntIndex),
							!mKeyboard->isKeyDown(OIS::KC_LSHIFT));
					}
				}
				...
				engine->entityMgr->AddMoveToCommandToSelectedEntities(clickLoc, !mKeyboard->isKeyDown(OIS::KC_LSHIFT));
			} else { //couldn't find entity
				std::cout << std::endl << "FAILURE: No entity near click! Nothing happened!" << std::endl;
			}
	...
	if (mid == OIS::MouseButtonID::MB_Left) {
		m_select_start_x = float(me.state.X.abs);
		m_select_start_y = float(me.state.Y.abs);
	}
...
bool InputMgr::mouseReleased(const OIS::MouseEvent& me, OIS::MouseButtonID mid) {
	if (mid == OIS::MouseButtonID::MB_Left && mKeyboard->isKeyDown(OIS::KC_RSHIFT)) {

		bool primary_selected = false;
		engine->entityMgr->ClearEntityGroup();

		for (float x = std::min(m_select_start_x, m_select_end_x); x < std::max(m_select_start_x, m_select_end_x); ++x) {
			for (float y = std::min(m_select_start_y, m_select_end_y); y < std::max(m_select_start_y, m_select_end_y); ++y) {

				//create ray and check intersection with xz plane
				Ogre::Ray ray = engine->gfxMgr->mCamera->getCameraToViewportRay(x / float(engine->gfxMgr->mWindow->getWidth()),
					y / float(engine->gfxMgr->mWindow->getHeight()));
				std::pair<bool, Ogre::Real> groundIntersection = ray.intersects(Ogre::Plane(Ogre::Vector3(0.f, 1.f, 0.f), 0.f));
				...
				if (groundIntersection.first) { //we have an intersection with ocean
					...
					if (nearestEntIndex >= 0) {
						if (!primary_selected) {
							engine->entityMgr->SelectEntity(nearestEntIndex);
							primary_selected = true;
						} else {
							engine->entityMgr->AddEntityToGroup(nearestEntIndex, false);
						}
					}
				}
			}
